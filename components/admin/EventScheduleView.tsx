...
import { ChevronDown, ChevronRight, Users, CheckCircle, Clock, XCircle, MapPin } from 'lucide-react'
...
// Parse "09:00 AM – 10:30 AM" → minutes-since-midnight for sorting
...
export default function EventScheduleView({ events, registrations }: Props) {
  ...
  // Group events by activity name → sorted by start time within each group
  const lanes = useMemo(() => {
    ...
    const result = Object.entries(byName).map(([name, slots]) => ({
      name,
      slots: [...slots].sort((a, b) => parseStartMinutes(a.slot_time) - parseStartMinutes(b.slot_time)),
      totalRegistrations: slots.reduce((s, e) => {
        const st = stats[e.id] ?? { confirmed: 0, waitlisted: 0, cancelled: 0 }
        return s + st.confirmed + st.waitlisted + st.cancelled
      }, 0),
      totalCapacity: slots.reduce((s, e) => s + e.max_participants, 0),
    }))
    // Sort lanes by earliest slot_time across their slots
    result.sort((a, b) =>
      parseStartMinutes(a.slots[0]?.slot_time ?? '') -
      parseStartMinutes(b.slots[0]?.slot_time ?? '')
    )
    return result
  }, [events, stats])
  ...
  // Determine earliest and latest slot for the day-view header
  const allTimes = events.map(e => parseStartMinutes(e.slot_time)).filter(t => t !== 9999)
  const dayStart = allTimes.length ? Math.min(...allTimes) : 420   // fallback 07:00
  const dayEnd   = allTimes.length ? Math.max(...allTimes) + 120 : 1320 // +2h buffer
  const daySpan  = dayEnd - dayStart || 1

  const pct = (slot: string) => ((parseStartMinutes(slot) - dayStart) / daySpan) * 100
    ...
    <div className="space-y-6">
      ...
      <div className="relative h-6 bg-slate-50 rounded-lg border border-slate-200 overflow-hidden">
        ...
        {Array.from({ length: Math.ceil(daySpan / 60) + 1 }, (_, i) => {
          ...
          return left <= 100 ? (
            <div key={i} className="absolute top-0 bottom-0 flex flex-col justify-center" style={{ left: `${left}%` }}>
      ...
      <div className="space-y-2">
        {lanes.map(lane => {
          const isOpen = !!expanded[lane.name]
          const fillPct = lane.totalCapacity > 0
            ? Math.min(100, Math.round((lane.totalRegistrations / lane.totalCapacity) * 100))
            : 0
          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Activity name */}
                <span className="font-semibold text-slate-800 w-44 shrink-0 truncate">
                  {lane.name}
                </span>

                {/* Mini timeline showing slot blocks */}
                <div className="relative flex-1 h-7 bg-slate-50 rounded-md overflow-hidden hidden sm:block">
                  {lane.slots.map(slot => {
                    const left = pct(slot.slot_time)
                    const st   = stats[slot.id] ?? { confirmed: 0, waitlisted: 0, cancelled: 0 }
                    const total = st.confirmed + st.waitlisted + st.cancelled
                    return (
                      <div
                        key={slot.id}
                        className={`absolute top-1 bottom-1 rounded flex items-center justify-center text-[10px] font-bold text-white min-w-[32px] px-1 ${AGE_BAR[slot.age_group]}`}
                        style={{ left: `${Math.min(left, 85)}%`, width: '12%' }}
                ...
                {/* Registration count + fill */}
                <div className="flex items-center gap-3 shrink-0 ml-auto">
                  <div className="text-right hidden md:block">
                    <p className="text-sm font-bold text-slate-700">{lane.totalRegistrations}</p>
                    <p className="text-xs text-slate-400">registered</p>
                  </div>
                  <div className="w-20">
                    <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${fillPct >= 90 ? 'bg-red-400' : fillPct >= 60 ? 'bg-orange-400' : 'bg-green-400'}`}
                        style={{ width: `${fillPct}%` }}
                      />
                    </div>
                    <p className="text-[10px] text-slate-400 text-right mt-0.5">{fillPct}% full</p>
                  </div>
                </div>
              </button>

              {/* ── Expanded: one row per age-group slot ── */}
              {isOpen && (
                <div className="border-t border-slate-100 divide-y divide-slate-50">
                  {lane.slots.map(slot => {
                    ...
                    const slotFill = cap > 0 ? Math.min(100, Math.round((st.confirmed / cap) * 100)) : 0
                    const isActive = slot.is_active

                    return (
                      <div key={slot.id}
                        className={`flex flex-col sm:flex-row sm:items-center gap-4 px-5 py-4 transition-colors ${!isActive ? 'opacity-50 bg-slate-50' : 'hover:bg-slate-50/60'}`}
                      >
                        {/* Age group chip + time + location */}
                        <div className="flex items-start gap-3 sm:w-80 shrink-0">
                        ...
                        {/* Stacked bar + capacity */}
                        <div className="flex-1 min-w-0">
                          {total > 0 ? (
                            <div className="flex h-4 rounded overflow-hidden gap-px" title={`${total} of ${cap} registered`}>
      ...
      <div className="flex flex-wrap gap-4 text-xs text-slate-500 pt-2 border-t border-slate-100">
        ...
        {Object.entries(AGE_BAR).map(([k, c]) => (
          <LegendDot key={k} color={c} label={AGE_GROUP_LABEL[k]} />
        ))}
      </div>
    </div>
  )
...
function LegendDot({ color, label }: { color: string; label: string }) {
  return (
    <span className="flex items-center gap-1.5">
      <span className={`w-3 h-3 rounded-sm ${color} border border-white/50`} />
      {label}
    </span>
  )
}
